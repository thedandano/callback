# profile-graph Specification

## Purpose

Define the topology, interface, and behavior of the `profile_graph` — a 5-node cyclic LangGraph
state machine that manages the user's skills and accomplishments profile. The graph supports
onboarding, interactive story creation, and recompilation cycles to drain orphaned skills.
Interrupts after `onboard` and `create_story` to allow the caller to provide user input.
## Requirements
