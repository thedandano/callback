...
def test_split_timeline_entries_single_heading_without_date_is_organization():
    assert _split_timeline_entries("Amazon, San Diego, CA\n• Built pricing systems") == [
        {
            "organization": "Amazon, San Diego, CA",
            "role": None,
            "date": None,
...
def test_render_resume_returns_error_on_missing_output_parent(tmp_path):
    ...
    assert render_resume({"name": "Dan Sedano"}, output_path) == {
        "success": False,
...
def test_render_resume_removes_empty_pdf_on_failure(tmp_path, monkeypatch):
    ...
    result = render_resume({"name": "Dan Sedano"}, str(output_path))
...
def test_render_html_structures_title_contact_and_timeline_rows():
    html = _render_html(
        {
            ...
            "email": "dansedano.dev@gmail.com",
            "linkedin": "www.linkedin.com/in/sdedano",
    ...
    assert "San Diego, California" in html
    assert '<span class="contact-link">www.linkedin.com/in/sdedano</span>' in html
    assert '<div class="section-header">SOFTWARE ENGINEER – BACKEND & AI</div>' in html
    assert '<div class="entry-organization">Amazon, San Diego, CA</div>' in html
...
def test_render_html_keeps_summary_divider_when_title_is_absent():
    ...
    assert html.index("San Diego, California") < divider_index
...
def test_render_html_uses_compact_internal_line_height_and_room_between_groups():
    html = _render_html({"name": "Dan Sedano", "summary": "Backend engineer."})
