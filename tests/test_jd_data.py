...
PARTIAL_JD_JSON = '{"title":"Backend Engineer","company":"ExampleCo","required":["Python"]}'
...
class TestJDDataModel:
    ...
    def test_optional_fields_can_be_omitted(self):
        ...
        assert data["title"] == PARTIAL_JD["title"]
        assert data["company"] == PARTIAL_JD["company"]
        assert data["required"] == PARTIAL_JD["required"]
        assert data.get("preferred") in (None, [])
        assert data.get("location") is None
        assert data.get("team") is None
        assert data.get("key_responsibilities") in (None, [])
        assert data.get("pay_range_min") is None
        assert data.get("pay_range_max") is None
    ...
    def test_unsupported_seniority_is_rejected(self):
        with pytest.raises(JDDataError) as exc_info:
            parse_jd_json(
                ...
                '"required":["Python"],"seniority":"principal"}'
            )
