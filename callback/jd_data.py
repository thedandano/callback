...
2. A disjunction - two or more NAMED alternatives where only one is needed - goes into a required_any GROUP: a list of the named alternatives, appended to the required_any list of groups. Do NOT dump these into preferred. Recognize a disjunction from ANY of these signals, not just the word "or": "X or Y" (exactly two alternatives is still a group, not two separate flat terms), a slash between named alternatives ("AWS/GCP" -> ["AWS","GCP"]), "such as X, Y, Z" / "like X, Y, Z" (with or without a trailing "etc."), or "one or more of X, Y, Z". The opposite case: a list joined by "and", where the posting expects ALL of the named items together, is NOT a disjunction - keep those as separate flat atomic terms, one per item. For "X or some other Y" / "X or equivalent" phrasing that names only ONE concrete alternative, extract just X as a normal atomic required term - do NOT create a one-member group for it (a one-member group is scoring-identical to a scalar).
...
5. A keyword is a NAMED technology, tool, framework, platform, language, credential, or named methodology or domain, explicitly stated in the JD - include every one, do not filter by perceived importance. Bare activity words a reader could guess from the job title alone are NOT keywords - e.g. "training", "evaluating", "tooling", "cloud", "analytics", "data processing", "software engineering fundamentals".
...
Examples:
  ...
  -> {"title":"Software Engineer","company":"Acme Corp","required_any":[["AWS","GCP"]],"preferred":["Kubernetes","Terraform"]}"""  # noqa: E501
