...
The consequence: **every grouping decision is yours, made at `create_story` time — but it is not frozen.** Each story is a markdown file; the user (or you, by editing the file) can correct `job_title`, `tags`, or the body, and the next `compile_profile` reads the corrected file. What re-onboarding does *not* do is re-read the accomplishments source: an authoring mistake (e.g. tagging a Projects entry with a Freelancing role) survives until someone edits the story page or creates a corrected story. This is why the workflow below gates compilation behind an explicit user check. When you or the user regroup a story by editing `job_title`, change `type` in the same edit (`project` when `job_title` is `Project`, otherwise `story`): `submit_keywords` picks project candidates from `type` alone, and `compile_profile` never rewrites a story page, so a stale `type` silently misclassifies the story (compile logs a warning when the two disagree).
