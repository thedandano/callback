...
17. Parent records every outcome and updates automation memory before the final response. If `.callback/config.json` sets `ledger_db`, also record every real application, application confirmation, or recruiter resume submission as a contact in the job search ledger; if `edd_xlsx` is also set, export the unemployment-compatible Excel workbook too. If `ledger_db` isn't set, skip this step — do not mention the ledger to the user. If `edd_xlsx` isn't set, skip only the export, not the ledger recording.
