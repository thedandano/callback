...
Get the user a job by maximizing qualified interview chances: find and score Anytown-area or remote-workable software/AI engineering roles inside the user's core domains, avoid duplicate or spammy applications, and never submit an application until the user has reviewed and explicitly approved it in the current turn.

## Candidate Preferences

- Location: user lives in Anytown. Curate only Anytown-area in-person, Anytown-area hybrid, or remote roles that are explicitly workable from Anytown.
- Treat location as a hard curation gate. Do not curate non-San-Diego onsite or hybrid roles.
- Do not curate remote roles that are tied to another metro/state/country, require residence outside Anytown/California, or are ambiguous about whether the user can work from Anytown.
- Do not reject a strong role just because it is in-person or hybrid when it is local to Anytown.
...
- Automation memory: `~/REDACTED/memory.md`
- Automation config: `~/REDACTED/automation.toml`
- Canonical record CSV (one only): `~/REDACTED/data/Codex_Job_application_record.csv`
- Job search ledger repo: `~/REDACTED`
- Canonical ledger DB (one only): `~/REDACTED/data/job_search_ledger.sqlite3`
- Canonical Excel tracker (one only): `~/REDACTED/data/Job_application_tracker.xlsx`
- Per-role run artifacts: `~/REDACTED/applications/<YYYY-MM-DD>/<company-role-slug>/`
- Dated backups/exports only under `archive/`: `~/REDACTED/archive/csv_backups/`, `.../archive/xlsx_exports/`, `.../archive/sqlite_backups/`, `.../archive/screenshots/`
- User inputs: `~/REDACTED`
- Callback repo: `~/workplace/callback`
...
- Google Jobs curator agent: searches Google Jobs or current web search results for jobs that appeared within the last 3 days only. It targets Anytown-area or Anytown-workable remote SWE, Software Engineer, AI Engineer, GenAI/LLM/RAG, backend/platform/data engineering, AWS/Python/Java/TypeScript roles. It returns discovery leads only; it must not score, tailor, submit, label, archive, or reply. Google Jobs cards, search-result snippets, and job-board summary cards are discovery leads only, even when they look complete.
- FAANG curator agent: searches current official careers/source pages for Meta, Amazon, Apple, Acme, and Google. It targets remote or Anytown-area SWE, Software Engineer, Software Engineering, AI Engineer, GenAI/ML/platform/backend/data roles. It returns FAANG source leads only; it must not score or tailor. Acme-specific referral, source, and level rules still apply inside this broader FAANG agent.
...
- `Source URL`: employer careers URL, official ATS URL, Acme source URL, or directly opened third-party full-JD URL when known. Use `Needs source - manual lookup` if only a snippet/email/card URL is available.
...
- Google Jobs: Search Google Jobs or current web search results every run for jobs that appeared within the last 3 days. Use query/time filters that make the 3-day recency constraint explicit. Search for Anytown and remote-workable SWE, Software Engineer, AI Engineer, GenAI/LLM/RAG, backend/platform/data, AWS, Python, Java, and TypeScript roles. Treat Google Jobs cards and snippets as discovery only. Before scoring, open the employer careers page, official ATS page, or directly opened third-party full-JD page and confirm it exposes the complete current JD. If no complete source can be opened, record `Needs source - manual lookup` and do not score.
- FAANG careers: Search current official source/careers pages for Meta, Amazon, Apple, Acme, and Google every run. Prefer the employer's official source over third-party mirrors. Capture canonical URL, requisition/job ID, work type, location, salary range, posting date, and visible level when present. Apply the same source-only scoring, location, compensation-priority, and core-domain rules as all other roles.
- FAANG level mapping: use Levels.fyi as a sanity check when level mapping matters. Plausible target bands are Meta E4/E5, Google L4/L5, Amazon L5/SDE II, Apple ICT3/ICT4, and Acme L4/E4/Software Engineer II. Treat staff/principal bands as hard seniority blockers unless the user explicitly asks for a stretch.
- Acme level mapping: use Levels.fyi as a sanity check. Amazon L5 / SDE II maps closest to Acme L4 / E4 / Software Engineer II, not Acme L5. Do not curate or score Acme L5 / E5 / Senior Software Engineer roles by default. Acme L6+ / E6+ is staff-level and should be skipped as a seniority blocker. Only score Acme L5+ roles when the user explicitly names a specific referral stretch in the current turn.
- For plausible Acme L4 / E4 / Software Engineer II or clearly mid-level Acme roles inside the core domains, score and tailor even when the score would normally be below 70; the goal is to give the user enough evidence and artifacts to ask their friend for a referral.
...
8. For new plausible mid-level Acme roles inside the core domains, parent surfaces them immediately in the final summary as referral leads so the user can ask their friend for a referral before applying. Use status `Referral lead - ask friend` when the role should pause for referral outreach before direct application, but still require a full Acme source or full source-page text before scoring. For non-Acme FAANG roles, use normal review-gate rules unless the user explicitly names a referral path.
...
10. Each Callback role agent scores every plausible Anytown-area SWE, Software Engineer, AI Engineer, or adjacent software/AI role inside the core domains with Callback against the actual compiled profile/resume unless there is a hard blocker such as active clearance, internship/new-grad mismatch, closed posting, duplicate, non-San-Diego location, off-domain mismatch, Acme L5+/senior mismatch, or missing full source. Score and tailor plausible mid-level Acme referral leads regardless of the normal 70-point gate after full-source validation. Low, ambiguous, or missing compensation is acceptable for scoring, but must be called out as a compensation risk / verify-before-applying item. Do not use quick impressions as the final fit filter, but do use compensation as a prioritization signal after source, location, and core-domain gates.
...
14. If tailored score is under 70, parent records it and does not apply. For plausible mid-level Acme referral leads, still keep the tailored artifacts and status `Referral lead - ask friend` instead of filtering the role out.
15. If tailored score is 70 or higher, parent stages it for user review. Set status to `Needs review - not applied`, link the tailored resume/cover letter if generated, and summarize why it is worth reviewing plus any remaining mismatch risk. For mid-level Acme roles, still preserve the referral-first note and referral outreach action.
...
- Run ledger commands from `~/REDACTED` with `uv run job-search-ledger --db ~/REDACTED/data/job_search_ledger.sqlite3 ...`.
...
- After recording a reportable contact, run `export-excel` to update the canonical tracker `~/REDACTED/data/Job_application_tracker.xlsx` in place, with its first columns matching the unemployment/EDD schema. Do not create dated or run-suffixed xlsx variants alongside it. If a dated export is genuinely needed (e.g. before a risky bulk edit), write it into `~/REDACTED/archive/xlsx_exports/`, never to the project root or `data/`. If writing the canonical tracker in place is blocked, write the dated export under `archive/xlsx_exports/` and state that a privacy/lock issue prevented updating the canonical workbook directly.
...
- Mid-level Acme referral leads found, if any, including tailored artifacts and score even when under 70
...
| Jobs found | Curated | Scored | Applied | Max score | Mean score | Min score | Status emails | Source-manual leads | Acme referral leads |
| ---------: | ------: | -----: | ------: | --------: | ---------: | --------: | ------------: | ------------------: | ---------------------: |
...
| FAANG curator       |              0 |            0 |                   0 | Meta, Amazon, Apple, Acme, Google official sources. |
